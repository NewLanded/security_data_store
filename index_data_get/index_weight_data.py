...
from util_base.date_util import convert_str_to_datetime, get_date_range, convert_datetime_to_str
from util_base.db_util import get_connection, store_failed_message
from util_data.date import Date
from util_data.index import Index

ts.set_token(PRO_KEY)
pro = ts.pro_api()

"""
现在这个接口取得是沪深300的指数权重, 还不清楚怎么弄其他指数, 
没能解决删除在插入新数据失败的时候删除旧数据的问题
而且hs300的指数也取不到最近几天的--好像是只在月末取
"""
